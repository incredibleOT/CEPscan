...
class constrainedEffectivePotential
{
	...
	//whether to interprete the gaussian part as
	//1/2 phi^dagger[ 2 - 4 lambda_N - 4 kappa\sum(cos(p_mu) + 8 lambda (m^2 + s^2) + 18 lambda_6_N ( m^4 + s^4 + 6 m^2 s^2) ] phi
	// this will increase the costs of the computation of the functions, since an additional expression has to be evaluated
	...
	//double computeBosonicPropagatorSum_qad(); not used anymore
	double computeBosonicPropagatorSum_fromStoredSumOfCos();
	
	
	//++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	//computes only the fuction
	//++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	double computeConstrainedEffectivePotential_onlyFunction(const double magnetization, const double staggeredMagnetization);
	
	//computation of the fermionic contribution, just computes the sum of logs as in philipp's thesis 4.35
	...
	//++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	//computes function and gradient at the same time
	//++++++++++++++++++++++++++++++++++++++++++++++++++++++++
